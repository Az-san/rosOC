...
class StartTrial(
    ...
    def execute(
        ...
        #2024/10/18 川原 追記　未完
        #fire_points = self._lib["map"].getFirePosition()#lib_map 未完
        #fire_points = [[2.63, -2.5], [7.3, 0]]
        fire_points = [[2.63, -2.5], [9.5, -3.85]]
        rospy.set_param(rospy.get_name() + "/fire_points", fire_points) #交差点通過回数
        ###
...
class CheckIntersection(
    ...
    def __init__(
        ...
        smach.State.__init__(
            self,
            outcomes = ["true", "false", "loop", "except", "find"]#2024/10/24川原追記
        )
    ...
    def execute(
        ...
        #追記 川原（2024/10/21）
        fire_points = rospy.get_param(rospy.get_name() + "/fire_points", [])
        ###
        ...
        #for文で現在座標と炎座標群それぞれの距離算出,正面
        #print(position_meter)
        #print(fire_points[0][0])
        #print(inf_rad_sorted)
        
        #2024/10/29川原追記 ロボットの正面方角を入手
        lin_rad = self._lib["map"].getRobotNSEW(
            position_meter[2],
        )
        
        point = self._lib["scan"].getLaserInfPointMeter(
            [lin_rad],
            position_meter,
            distance = 0.5
        )
        """
        #ロボット正面方向0.5m先座標取得
        min_rob_rad = [inf_rad_sorted[i] for i in range(len(inf_rad_sorted)) if 0.70 < math.cos(inf_rad_sorted[i] - position_meter[2])]
        #print(min_lob_rad)
        point = self._lib["scan"].getLaserInfPointMeter(
            min_rob_rad,
            position_meter,
        ...
        #火の座標と正面座標を比較して0.5m以内ならfind
        #if min_rob_rad != []:
        if lin_rad != None:
            for i in range(len(fire_points)):
                #print(i)
                #print(position_meter[0])
                #print(fire_points[i][0])
            
                x = (point[0][0] - fire_points[i][0])
                y = (point[0][1] - fire_points[i][1])
                distance = (x ** 2) + (y ** 2)
                #print(distance)
            
                if distance < 0.5: 
                    rospy.set_param(rospy.get_name() + "/fire_num", i)
                    rospy.set_param(rospy.get_name() + "/lin_rad", lin_rad)
                    ...
                    self._lib["nav"].sendMeterGoal(
                        [position_meter[0], position_meter[1]],
                        lin_rad
                    )
                    rospy.sleep(1.0)
                    self._lib["record"].saveMap()
    ...
    def execute(
        ...
        if self._lib["scan"].getAheadDistance() < 1.0:
            ...
            self._lib["nav"].sendRunTwist(
                ...
                position_meter[2],
                #lin_speed = 0.3,#2024/10/29川原追記
                #rad_speed = math.pi / 30.0#2024/10/29川原追記
            )
...
class MakeHumanDecideDirection(
    ...
    def execute(
        ...
        while True:
            ...
            human_direction = self._lib["com"].readSocketsDirection(loc = path, latency = 3)#2024/10/22 川原 追記
        ...
        print(decided_direction)
        print(decided_direction_idx)
        self._lib["if"].showDirection(decided_direction_idx)
        rospy.sleep(1.0)
        ###
...
class MakeHumanDecideMagic(
	...
	#炎座標群取得
        fire_points = rospy.get_param(rospy.get_name() + "/fire_points", [])
        fire_num = rospy.get_param(rospy.get_name() + "/fire_num", -1)
        
        lin_rad = rospy.get_param(rospy.get_name() + "/lin_rad", 0)
        ...
        while True:
            path = roslib.packages.get_pkg_dir("robot_pkg") + "/script"
            human_decide = self._lib["com"].readSocketsFire(loc = path, latency = 3)

            if human_decide == None:
                print("input timeout ...")
                continue
            else:
                decided_fire = human_decide #[0or1]
                break

        print("chime!!!!!!!")
        p = subprocess.Popen(['python3','/home/user/rosOC/src/robot_pkg/script/import/chime.py'])

        rospy.set_param(rospy.get_name() + "/human_decided_fire", decided_fire) # [上、下、右、左]
        
        self._lib["if"].showFire(decided_fire)
        rospy.sleep(1.0)
        if decided_fire == 0:#引き返し
            pixels = self._lib["map"].convMeters2Pixels([fire_points[fire_num]])
            point_pixel = self._lib["map"].getRobotPointPixel()
            print(position_meter)
            print(point_pixel)
            print(fire_points[fire_num])
            print(pixels)
            self._lib["if"].fireUpdate(pixels[0])#ミニマップに炎座標送信
            ...
            """
            self._lib["nav"].sendRotationTwist(lin_rad - math.pi, position_meter[2])
            """
            
            
        elif decided_fire == 1:#魔法
            fire_points.pop(fire_num)
            rospy.set_param(rospy.get_name() + "/fire_points", fire_points) #交差点通過回数
            #ロボットに正面を向かせる
            self._lib["nav"].sendMeterGoal(
                [position_meter[0], position_meter[1]],
...
class MakeRobotDecideDirection(
    ...
    def execute(
        ...
        # GUIの変更
        #self._lib["if"].changeGUI("move")#2024/10/25川原追記
...
class DecideDirection(
    ...
    def execute(
        ...
        self._lib["if"].showDirection(4)#2024/10/28 川原追記
        self._lib["if"].changeGUI("move")#2024/10/28 川原追記
