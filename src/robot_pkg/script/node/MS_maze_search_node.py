...
class StateMachine:
    ...
    def __init__(
        ...
        with self._ssm:
            ...
            smach.StateMachine.add(
                "CheckIntersection", #炎との分岐を追加する
                CheckIntersection(self._lib),
                transitions = {
                    "true":"MakeDirectionCandidates",
                    "find":"MakeHumanDecideMagic",#炎分岐
                    "false":"GoAhead",
            ...
            #追記 川原（2024/10/16）
            smach.StateMachine.add(
                "MakeHumanDecideMagic", #追加（魔法行使）
                MakeHumanDecideMagic(self._lib),
                transitions = {
                    "next":"CheckIntersection",
                    "loop":"MakeHumanDecideMagic",
                    "except":"Except"
                }
            )
            #追記 川原（2024/10/16）
            ...
            smach.StateMachine.add(
                "DecideDirection", #MHDDに機能を纏めるため削除
            ...
            smach.StateMachine.add(
                "CheckErrP", #MHDDにArduino通信や脳波取得機能を纏めるため削除
                CheckErrP(self._lib),
                transitions = {
                    "next":"UpdateQTable",
                    "loop":"CheckErrP",
