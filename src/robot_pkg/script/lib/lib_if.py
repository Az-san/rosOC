...
class LibIF:
    ...
    def showDirection(
        ...
        self.pub_direction.publish(direction_num)
            
    def showFire(
        self,
        action_num
    ):
        self.pub_action.publish(action_num)
        
    def fireUpdate(
        self,
        pixels
    ):
        fire_str = [str(n) for n in pixels]
        self.pub_fire.publish(",".join(fire_str))
        ####
