...
#==================================================
    ...
    ## @fn convMeters2Pixels
    ## @brief Meters単位をPixels単位にする
    ## @param
    ## @return

    #==================================================
    def convMeters2Pixels(
        self,
        meters
    ):
        origin_pixel = self.getMapOriginPixel()
        ...
        for i in range(len(meters)):
            #x = (origin_Meter[0] - meters[i][0]) / self.sub_map_resolution
            #y = (origin_Meter[1] - meters[i][1]) / self.sub_map_resolution
            x = int(origin_pixel[0] - meters[i][1]/self.sub_map_data.info.resolution)
            if x<0:
                x += self.sub_map_data.info.height
            y = int(origin_pixel[1] - meters[i][0]/self.sub_map_data.info.resolution)
            if y<0:
                y += self.sub_map_data.info.width
    ...
    def getRobotNSEW(
        ...
        dif = 999
        rad = rad % (math.pi * 2)
        for i in range(5):#0,90,180,270,360(360があるのは357のときなどに最小が90とならないようにするため)
            map_direction_rad = i * (math.pi / 2)
            print(map_direction_rad)
            robot_direction_dif = abs(rad - map_direction_rad)
            print(robot_direction_dif)
            if dif > robot_direction_dif:
                if map_direction_rad == 4 * (math.pi / 2):
                    map_direction_rad = 0
                dif = robot_direction_dif
                lin_rad = map_direction_rad
        print(lin_rad)
